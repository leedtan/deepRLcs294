...
def conv2d_input(input_, output_dim, wb,d_h=2, d_w=2, stddev=0.02):
    w = wb[0]
    b = wb[1]
...
def conv2d_bn_relu_trn_tst(input_trn, output_dim, k, d, stddev, name='conv2d', input_tst = None):
    bn = batch_norm(name=name)
    wb = make_wb_conv2d(input_trn, output_dim, k, k, stddev, name = name + 'wb')
    trn = lrelu(bn(conv2d_input(input_trn, output_dim, wb, d, d, stddev)))
    if input_tst == None:
        input_tst = input_trn
    
    tst = lrelu(bn(conv2d_input(input_tst, output_dim, wb, d, d, stddev), train=False))
    return trn, tst
