...
def learn(env,
	...
	cur_q_trn, cur_q_tst = current_q_func[0], current_q_func[1]

	q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')

	target_q_func = q_func(obs_tp1_float, num_actions, scope="target_q_func", reuse=False)[0]
	target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')

	act_t = tf.one_hot(act_t_ph, depth=num_actions, dtype=tf.float32, name="action_one_hot")
	q_act_t = tf.reduce_sum(act_t*cur_q_trn, axis=1)

	y = rew_t_ph + gamma * tf.reduce_max(target_q_func, reduction_indices=[1])
	
	t_vars = tf.trainable_variables()
	l2_loss = tf.reduce_sum([tf.reduce_sum(tf.square(var)) * 1e-8 for var in t_vars])

	total_error = tf.square(y - q_act_t) + l2_loss
	...
	for t in itertools.count():
		...
		if not model_initialized or random.random() < epsilon:
			...
			q_vals = session.run(cur_q_tst, {obs_t_ph: input_batch[None, :]})
